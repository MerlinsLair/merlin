...
use merlin_testing::{Module, RunnerError, Wasm};
...
test_set_route!(
    ...
    // r#"Invalid Pool Route: "denom umer is not in pool id 1": execute wasm contract failed"#,
    ...
    msg = ExecuteMsg::SetRoute {
        ...
        pool_route: vec![
            SwapAmountInRoute {
                pool_id: 1, // umer/uion
                token_out_denom: "umer".to_string(),
            },
        ],
    }
);
...
test_set_route!(
    non_existant_pool
    should failed_with
    r#"Invalid Pool Route: "denom umer is not in pool id 3": execute wasm contract failed"#,
...
fn test_set_route_failed_case(sender: Sender, msg: ExecuteMsg, expected_error: &str) {
    ...
    let sender = match sender {
        ...
        Sender::NonOwner => {
            let initial_balance = [
                Coin::new(1_000_000_000_000, "umer"),
