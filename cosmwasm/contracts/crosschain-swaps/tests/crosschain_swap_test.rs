...
use merlin_testing::{Account, Bank, Module, Wasm};
...
fn crosschain_swap() {
    ...
    let msg = CrossChainExecute::MerlinSwap {
        swap_amount,
        output_denom,
        ...
        receiver: "mer1l4u56l7cvx8n0n6c7w650k02vz67qudjlcut89".to_string(),
        on_failed_delivery: FailedDeliveryAction::DoNothing,
        next_memo: None,
    };
    let funds: &[Coin] = &[Coin::new(10000, "umer")];
    println!("{}", serde_json_wasm::to_string(&msg).unwrap());
    let _res = wasm.execute(&crosschain_address, &msg, funds, &sender);
    //dbg!(&res);

    // This test cannot be completed until we have ibc tests on merlin testing.
