...
func (suite *StargateTestSuite) SetupTest() {
	...
	suite.ctx = suite.app.BaseApp.NewContext(false, tmproto.Header{Height: 1, ChainID: "merlin-1", Time: time.Now().UTC()})
}
...
func (suite *StargateTestSuite) TestStargateQuerier() {
	testCases := []struct {
		...
		},
		{
			...
			path: "/merlin.gamm.v2.Query/SpotPrice",
			testSetup: func() {
		...
		},
		{
			...
			path: "/merlin.lockup.Query/AccountLockedLongerDuration",
			requestData: func() []byte {
		...
		},
		{
			...
			testSetup: func() {
				accAddr, err := sdk.AccAddressFromBech32("mer1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44")
				suite.Require().NoError(err)
			...
			requestData: func() []byte {
				bankrequest := banktypes.QueryAllBalancesRequest{
					Address: "mer1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44",
				}
