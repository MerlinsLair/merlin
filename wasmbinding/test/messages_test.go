...
import (
	...
	"github.com/merlinslair/merlin/wasmbinding"
...
func TestCreateDenom(t *testing.T) {
	...
	fundAccount(t, ctx, merlin, actor, actorAmount)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformCreateDenom(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, actor, spec.createDenom)
			// then
			if spec.expErr {
...
func TestChangeAdmin(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// Setup
			merlin, ctx := SetupCustomApp(t, tokenCreator)

			// Fund actor with 100 base denom creation fees
			actorAmount := sdk.NewCoins(sdk.NewCoin(types.DefaultParams().DenomCreationFee[0].Denom, types.DefaultParams().DenomCreationFee[0].Amount.MulRaw(100)))
			fundAccount(t, ctx, merlin, tokenCreator, actorAmount)

			err := wasmbinding.PerformCreateDenom(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, tokenCreator, &bindings.CreateDenom{
				Subdenom: validDenom,
			})
			require.NoError(t, err)

			err = wasmbinding.ChangeAdmin(merlin.TokenFactoryKeeper, ctx, spec.actor, spec.changeAdmin)
...
func TestMint(t *testing.T) {
	...
	fundAccount(t, ctx, merlin, creator, tokenCreationFeeAmt)

	// Create denoms for valid mint tests
	validDenom := bindings.CreateDenom{
		Subdenom: "MOON",
	}
	err := wasmbinding.PerformCreateDenom(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, creator, &validDenom)
	require.NoError(t, err)

	emptyDenom := bindings.CreateDenom{
		Subdenom: "",
	}
	err = wasmbinding.PerformCreateDenom(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, creator, &emptyDenom)
	require.NoError(t, err)
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			// when
			gotErr := wasmbinding.PerformMint(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, creator, spec.mint)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
...
func TestBurn(t *testing.T) {
	...
	for name, spec := range specs {
		t.Run(name, func(t *testing.T) {
			...
			err := wasmbinding.PerformMint(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, creator, mintBinding)
			require.NoError(t, err)
			...
			err = wasmbinding.PerformMint(merlin.TokenFactoryKeeper, merlin.BankKeeper, ctx, creator, emptyDenomMintBinding)
			require.NoError(t, err)

			// when
			gotErr := wasmbinding.PerformBurn(merlin.TokenFactoryKeeper, ctx, creator, spec.burn)
			// then
			if spec.expErr {
				require.Error(t, gotErr)
				return
