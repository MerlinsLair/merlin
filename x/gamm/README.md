...
```sh
merlin tx gamm create-pool [config-file] --from --chain-id
```
...
```sh
merlin tx gamm join-pool --pool-id --max-amounts-in --share-amount-out --from --chain-id
```
...
```sh
merlin tx gamm exit-pool --pool-id --min-amounts-out --share-amount-in --from --chain-id
```
...
```sh
merlin tx gamm join-swap-extern-amount-in [token-in] [share-out-min-amount] --from --pool-id --chain-id
```
...
```sh
merlin tx gamm exit-swap-extern-amount-out [token-out] [share-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlin tx gamm join-swap-share-amount-out [token-in-denom] [share-out-amount] [token-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlin tx gamm exit-swap-share-amount-in [token-out-denom] [share-in-amount] [token-out-min-amount] --pool-id --from --chain-id
```
...
```sh
merlin tx gamm swap-exact-amount-in [token-in] [token-out-min-amount] --pool-id --from --chain-id
```
...
```sh
merlin tx gamm swap-exact-amount-out [token-out] [token-out-max-amount] --swap-route-pool-ids --from --chain-id
```
...
```sh
merlin query gamm estimate-swap-exact-amount-in <poolID> <sender> <tokenIn> [flags]
```
...
```sh
merlin query gamm estimate-swap-exact-amount-out <poolID> <sender> <tokenOut> [flags]
```
...
```sh
merlin query gamm pool <poolID> [flags]
```
...
```sh
merlin query gamm pool-assets <poolID> [flags]
```
...
```sh
merlin query gamm pool-assets 1
```
...
```sh
merlin query gamm pool-params <poolID> [flags]
```
...
```sh
merlin query gamm pool-params 1
```
...
```sh
merlin query gamm total-liquidity
```
...
```sh
merlin query gamm total-share <poolID> [flags]
```
...
```sh
merlin query gamm total-share 1
```
