...
The ``GAMM`` module (**G**eneralized **A**utomated **M**arket **M**aker) provides the logic to create and interact with liquidity pools on the Merlin DEX.
...
- Stableswap pools have liquidity centered around a given spot price. See [here](https://github.com/merlinslair/merlin/blob/main/x/gamm/pool-models/stableswap/README.md) for the spec of the Merlin implementation.
...
[Spot price](https://github.com/merlinslair/merlin/blob/main/x/gamm/keeper/swap.go)

#### Multi-Hop

The multi-hop logic is handled via `x/poolmanager` module.
Please see for details:
- https://github.com/merlinslair/merlin/blob/main/x/poolmanager/README.md
...
3. **FutureGovernor** -
    Merlin plans to allow every pool to act as a DAO, with its own governance in a future upgrade. To facilitate this transition, we allow pools to specify who the governor should be as a string. There are currently 3 options for the future governor.
...
[MsgCreateBalancerPool](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/pool-models/balancer/tx.proto#L16-L26)

### MsgJoinPool

[MsgJoinPool](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L27-L39)

### MsgExitPool

[MsgExitPool](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L44-L57)

### MsgSwapExactAmountIn

[MsgSwapExactAmountIn](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L68-L80)
...
[MsgSwapExactAmountOut](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L90-L102)
...
[MsgJoinSwapExternAmountIn](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L107-L119)

#### MsgJoinSwapShareAmountOut

[MsgJoinSwapShareAmountOut](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L124-L138)

#### MsgExitSwapShareAmountIn

[MsgExitSwapShareAmountIn](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L143-L158)

#### MsgExitSwapExternAmountOut

[MsgExitSwapExternAmountOut](https://github.com/merlinslair/merlin/blob/v7.1.0/proto/merlin/gamm/v1beta1/tx.proto#L163-L175)
...
```sh
merlind tx gamm create-pool [config-file] --from --chain-id
```
...
```sh
merlind tx gamm create-pool --pool-file [config-file] --from WALLET_NAME --chain-id merlin-1
```
...
```json
{
 "weights": "5ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4,5umer",
 "initial-deposit": "499404ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4,500000umer",
 "swap-fee": "0.01",
 "exit-fee": "0.01",
 "future-governor": ""
}
...
```sh
merlind tx gamm join-pool --pool-id --max-amounts-in --share-amount-out --from --chain-id
```
...
```sh
merlind tx gamm join-pool --pool-id 3 --max-amounts-in 37753ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --share-amount-out 1227549469722224220 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm exit-pool --pool-id --min-amounts-out --share-amount-in --from --chain-id
```
...
```sh
merlind tx gamm exit-pool --pool-id 3 --min-amounts-out 33358ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --share-amount-in 1136326462628731195 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm join-swap-extern-amount-in [token-in] [share-out-min-amount] --from --pool-id --chain-id
```
...
```sh
merlind tx gamm join-swap-extern-amount-in 200000ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 3234812471272883046 --pool-id 3 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm exit-swap-extern-amount-out [token-out] [share-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlind tx gamm exit-swap-extern-amount-out 199430ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 3408979387886193586 --pool-id 3 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm join-swap-share-amount-out [token-in-denom] [share-out-amount] [token-in-max-amount] --pool-id --from --chain-id
```
...
```sh
merlind tx gamm join-swap-share-amount-out umer 14481270389710236872 312466 --pool-id 3 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm exit-swap-share-amount-in [token-out-denom] [share-in-amount] [token-out-min-amount] --pool-id --from --chain-id
```
...
```sh
merlind tx gamm exit-swap-share-amount-in umer 14563185400026723131 298548 --pool-id 3 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm swap-exact-amount-in [token-in] [token-out-min-amount] --pool-id --from --chain-id
```
...
```sh
merlind tx gamm swap-exact-amount-in 407239ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 140530 --swap-route-pool-ids 3 --swap-route-denoms umer --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind tx gamm swap-exact-amount-out [token-out] [token-out-max-amount] --swap-route-pool-ids --from --chain-id
```
...
```sh
merlind tx gamm swap-exact-amount-out 140530umer 407239 --swap-route-pool-ids 3 --swap-route-denoms ibc/1480B8FD20AD5FCAE81EA87584D269547DD4D436843C1D20F15E00EB64743EF4 --from WALLET_NAME --chain-id merlin-1
```
...
```sh
merlind query gamm estimate-swap-exact-amount-in <poolID> <sender> <tokenIn> [flags]
```
...
```sh
merlind query gamm estimate-swap-exact-amount-in 1 mer123nfq6m8f88m4g3sky570unsnk4zng4uqv7cm8 1000000umer --swap-route-pool-ids 1 --swap-route-denoms ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 
```
...
```sh
merlind query gamm estimate-swap-exact-amount-out <poolID> <sender> <tokenOut> [flags]
```
...
```sh
merlind query gamm estimate-swap-exact-amount-out 1 mer123nfq6m8f88m4g3sky570unsnk4zng4uqv7cm8 1000000ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2 --swap-route-pool-ids 1 --swap-route-denoms umer
```
...
```sh
merlind query gamm pool <poolID> [flags]
```
...
```sh
merlind query gamm pool-assets <poolID> [flags]
```
...
```sh
merlind query gamm pool-assets 1
```
...
```sh
merlind query gamm pool-params <poolID> [flags]
```
...
```sh
merlind query gamm pool-params 1
```
...
```sh
merlind query gamm spot-price 1 umer ibc/27394FB092D2ECCD56123C74F36E4C1F926001CEADA9CA97EA622B25F41E5EB2
```
...
```sh
merlind query gamm total-liquidity
```
...
```sh
merlind query gamm total-share <poolID> [flags]
```
...
```sh
merlind query gamm total-share 1
```
