...
import (
	...
	"github.com/merlinslair/merlin/x/gamm/v2types"
)
...
func (suite *KeeperTestSuite) TestCalcJoinPoolNoSwapShares() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			poolId,
			sdk.NewCoins(sdk.NewCoin("foo", sdk.NewInt(5000000)), sdk.NewCoin("bar", sdk.NewInt(5000000)), sdk.NewCoin("baz", sdk.NewInt(5000000)), sdk.NewCoin("umer", sdk.NewInt(5000000))),
			nil,
		},
		{
			"valid even multi asset join test case",
			poolId,
			sdk.NewCoins(sdk.NewCoin("foo", sdk.NewInt(500000)), sdk.NewCoin("bar", sdk.NewInt(1000000)), sdk.NewCoin("baz", sdk.NewInt(1500000)), sdk.NewCoin("umer", sdk.NewInt(2000000))),
			nil,
		},
		{
			"invalid single asset join test case",
			poolId,
			sdk.NewCoins(sdk.NewCoin("umer", sdk.NewInt(1000000))),
		...
		},
		{
			...
			poolId,
			sdk.NewCoins(sdk.NewCoin("umer", sdk.NewInt(10000)), sdk.NewCoin("bar", sdk.NewInt(10000))),
			errors.New("no-swap joins require LP'ing with all assets in pool"),
		},
	}
...
func (suite *KeeperTestSuite) TestQueryTotalPoolLiquidity() {
	...
	suite.Require().NoError(err)
	expectedCoins := sdk.NewCoins(sdk.NewCoin("foo", sdk.NewInt(5000000)), sdk.NewCoin("bar", sdk.NewInt(5000000)), sdk.NewCoin("baz", sdk.NewInt(5000000)), sdk.NewCoin("umer", sdk.NewInt(5000000)))
...
func (suite *KeeperTestSuite) TestQueryBalancerPoolTotalLiquidity() {
	...
	res, err = queryClient.TotalLiquidity(gocontext.Background(), &types.QueryTotalLiquidityRequest{})
	suite.Require().NoError(err)
	suite.Require().Equal("5000000bar,5000000baz,5000000foo,5000000umer", sdk.Coins(res.Liquidity).String())
}
