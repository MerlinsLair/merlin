...
import (
	...
	valPref "github.com/merlinslair/merlin/x/valset-pref"
)
...
func (suite *KeeperTestSuite) SetupLocks(delegator sdk.AccAddress) []lockuptypes.PeriodLock {
	...
	suite.Require().NoError(err)
	workingLock, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, merToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	suite.FundAcc(sdk.AccAddress([]byte("addr5---------------")), merToLock)
	lockWithDifferentOwner, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, sdk.AccAddress([]byte("addr5---------------")), merToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	suite.Require().NoError(err)
	maxDurationLock, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, merToLock, morethanTwoWeekDuration)
	suite.Require().NoError(err)

	locks = append(locks, maxDurationLock)

	// unbonding locks
	unbondingLocks, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, merToLock, twoWeekDuration)
	suite.Require().NoError(err)
	...
	// synthetic locks
	syntheticLocks, err := suite.App.LockupKeeper.CreateLock(suite.Ctx, delegator, merToLock, twoWeekDuration)
	suite.Require().NoError(err)

	err = suite.App.LockupKeeper.CreateSyntheticLockup(suite.Ctx, syntheticLocks.ID, "umer", time.Minute, true)
	suite.Require().NoError(err)
