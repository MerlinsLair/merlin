...
func (suite *KeeperTestSuite) TestSuperfluidDelegate() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			// get pre-superfluid delgations mer supply and supplyWithOffset
			...
			// ensure post-superfluid delegations mer supplywithoffset is the same while supply is not
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			for denom, multiplier := range tc.multipliersByDenom {
				suite.App.SuperfluidKeeper.SetMerEquivalentMultiplier(suite.Ctx, 2, denom, multiplier)
			}
			...
			for interAccIndex, intermediaryAcc := range intermediaryAccs {
				...
				expAmount := suite.App.SuperfluidKeeper.GetRiskAdjustedMerValue(suite.Ctx, asset, decAmt.RoundInt())
