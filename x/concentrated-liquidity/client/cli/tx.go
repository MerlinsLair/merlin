...
func NewCreateConcentratedPoolCmd() (*osmocli.TxCliDesc, *clmodel.MsgCreateConcentratedPool) {
	return &osmocli.TxCliDesc{
		...
		Example: "create-concentrated-pool uion umer 1 \"[-1]\" 0.01 --from val --chain-id merlin-1",
	}, &clmodel.MsgCreateConcentratedPool{}
}

func NewCreatePositionCmd() (*osmocli.TxCliDesc, *types.MsgCreatePosition) {
	return &osmocli.TxCliDesc{
		Use:                 "create-position [lower-tick] [upper-tick] [token-0] [token-1] [token-0-min-amount] [token-1-min-amount] [frozen-until]",
		Short:               "create or add to existing concentrated liquidity position",
		Example:             "create-position [-69082] 69082 1000000000umer 10000000uion 0 0 1675237890 --pool-id 1 --from val --chain-id merlin-1",
...
func NewWithdrawPositionCmd() (*osmocli.TxCliDesc, *types.MsgWithdrawPosition) {
	return &osmocli.TxCliDesc{
		...
		Example:             "withdraw-position [-69082] 69082 100317215 100 --pool-id 1 --from val --chain-id merlin-1",
		CustomFlagOverrides: poolIdFlagOverride,
		Flags:               osmocli.FlagDesc{RequiredFlags: []*flag.FlagSet{FlagSetJustPoolId()}},
...
func NewCollectFeesCmd() (*osmocli.TxCliDesc, *types.MsgCollectFees) {
	return &osmocli.TxCliDesc{
		...
		Example:             "collect-fees [-69082] 69082 --pool-id 1 --from val --chain-id merlin-1",
		CustomFlagOverrides: poolIdFlagOverride,
		Flags:               osmocli.FlagDesc{RequiredFlags: []*flag.FlagSet{FlagSetJustPoolId()}},
