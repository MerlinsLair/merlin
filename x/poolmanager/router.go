...
func (k Keeper) RouteExactAmountIn(
	...
	if k.isMerRoutedMultihop(ctx, route, routes[0].TokenOutDenom, tokenIn.Denom) {
		isMultiHopRouted = true
		routeSwapFee, sumOfSwapFees, err = k.getMerRoutedMultihopTotalSwapFee(ctx, route)
		if err != nil {
			return sdk.Int{}, err
		}
	}
	...
	for i, route := range routes {
		...
		// If we determined the route is an mer multi-hop and both routes are incentivized,
...
func (k Keeper) RouteExactAmountOut(ctx sdk.Context,
	...
	// total_swap_fee = total_swap_fee = max(swapfee1, swapfee2)
	// fee_per_pool = total_swap_fee * ((pool_fee) / (swapfee1 + swapfee2))
	if k.isMerRoutedMultihop(ctx, route, routes[0].TokenInDenom, tokenOut.Denom) {
		isMultiHopRouted = true
		routeSwapFee, sumOfSwapFees, err = k.getMerRoutedMultihopTotalSwapFee(ctx, route)
		if err != nil {
			return sdk.Int{}, err
	...
	if isMultiHopRouted {
		insExpected, err = k.createMerMultihopExpectedSwapOuts(ctx, routes, tokenOut, routeSwapFee, sumOfSwapFees)
	} else {
...
func (k Keeper) getMerRoutedMultihopTotalSwapFee(ctx sdk.Context, route types.MultihopRoute) (
...
// createMerMultihopExpectedSwapOuts does the same as createMultihopExpectedSwapOuts, however discounts the swap fee
