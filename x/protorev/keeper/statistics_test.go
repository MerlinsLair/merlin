...
func (suite *KeeperTestSuite) TestGetProfitsByDenom() {
	// Should be zero by default
	profits, err := suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.ZeroInt()), profits)

	// Pseudo execute a trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByDenom(suite.Ctx, types.MerlinDenomination, sdk.NewInt(9000))
	suite.Require().NoError(err)

	// Check the updated result
	profits, err = suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(9000)), profits)

	// Pseudo execute a second trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByDenom(suite.Ctx, types.MerlinDenomination, sdk.NewInt(5000))
	suite.Require().NoError(err)

	// Check the updated result after the second trade
	profits, err = suite.App.ProtoRevKeeper.GetProfitsByDenom(suite.Ctx, types.MerlinDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(14000)), profits)

	// Check the result of GetAllProfits
	allProfits := suite.App.ProtoRevKeeper.GetAllProfits(suite.Ctx)
	suite.Require().Equal([]*sdk.Coin{{Denom: types.MerlinDenomination, Amount: sdk.NewInt(14000)}}, allProfits)
	...
	// Check the result of GetAllProfits
	allProfits = suite.App.ProtoRevKeeper.GetAllProfits(suite.Ctx)
	suite.Require().Equal([]*sdk.Coin{{Denom: types.AtomDenomination, Amount: sdk.NewInt(1000)}, {Denom: types.MerlinDenomination, Amount: sdk.NewInt(14000)}}, allProfits)
}
...
func (suite *KeeperTestSuite) TestGetProfitsByRoute() {
	...
	profit, err := suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.ZeroInt()), profit)

	// Pseudo execute a trade
	err = suite.App.ProtoRevKeeper.UpdateProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinDenomination, sdk.NewInt(1000))
	suite.Require().NoError(err)

	// Check the updated result
	profit, err = suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{1, 2, 3}, types.MerlinDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(1000)), profit)

	// Check the result of GetAllProfitsByRoute
	profits = suite.App.ProtoRevKeeper.GetAllProfitsByRoute(suite.Ctx, []uint64{1, 2, 3})
	suite.Require().Equal([]*sdk.Coin{{Denom: types.MerlinDenomination, Amount: sdk.NewInt(1000)}}, profits)
	...
	// Check the result of GetAllProfitsByRoute
	profits = suite.App.ProtoRevKeeper.GetAllProfitsByRoute(suite.Ctx, []uint64{1, 2, 3})
	suite.Require().Contains(profits, &sdk.Coin{Denom: types.MerlinDenomination, Amount: sdk.NewInt(1000)})
...
func (suite *KeeperTestSuite) TestUpdateStatistics() {
	...
	err := suite.App.ProtoRevKeeper.UpdateStatistics(suite.Ctx,
		...
		types.MerlinDenomination, sdk.NewInt(1000),
	)
	suite.Require().NoError(err)
	...
	err = suite.App.ProtoRevKeeper.UpdateStatistics(suite.Ctx,
		...
		types.MerlinDenomination, sdk.NewInt(1100),
	)
	suite.Require().NoError(err)
	...
	// Check the result of GetProfitsByRoute
	profit, err = suite.App.ProtoRevKeeper.GetProfitsByRoute(suite.Ctx, []uint64{2, 3, 4}, types.MerlinDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(1100)), profit)
