...
func (suite *KeeperTestSuite) TestGetTokenPairArbRoutes() {
	...
	_, err := suite.App.ProtoRevKeeper.GetTokenPairArbRoutes(suite.Ctx, "mer", "abc")
...
func (suite *KeeperTestSuite) TestGetAllBaseDenoms() {
	...
	suite.Require().Equal(baseDenoms[0].Denom, types.MerlinDenomination)
	...
	suite.App.ProtoRevKeeper.SetBaseDenoms(suite.Ctx, []*types.BaseDenom{{Denom: "mer"}, {Denom: "atom"}, {Denom: "weth"}})
	baseDenoms, err = suite.App.ProtoRevKeeper.GetAllBaseDenoms(suite.Ctx)
	suite.Require().NoError(err)
	suite.Require().Equal(3, len(baseDenoms))
	suite.Require().Equal(baseDenoms[0].Denom, "mer")
...
func (suite *KeeperTestSuite) TestGetPoolForDenomPair() {
	...
	suite.App.ProtoRevKeeper.SetPoolForDenomPair(suite.Ctx, types.AtomDenomination, types.MerlinDenomination, 1000)
	pool, err := suite.App.ProtoRevKeeper.GetPoolForDenomPair(suite.Ctx, types.AtomDenomination, types.MerlinDenomination)
	suite.Require().NoError(err)
	...
	suite.App.ProtoRevKeeper.SetPoolForDenomPair(suite.Ctx, types.MerlinDenomination, types.AtomDenomination, 3000)
	pool, err = suite.App.ProtoRevKeeper.GetPoolForDenomPair(suite.Ctx, types.MerlinDenomination, types.AtomDenomination)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestGetDeveloperFees() {
	...
	merFees, err := suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, types.MerlinDenomination)
	suite.Require().Error(err)
	suite.Require().Equal(sdk.Coin{}, merFees)
	...
	err = suite.App.ProtoRevKeeper.SetDeveloperFees(suite.Ctx, sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(100)))
	suite.Require().NoError(err)
	...
	merFees, err = suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, types.MerlinDenomination)
	suite.Require().NoError(err)
	suite.Require().Equal(sdk.NewCoin(types.MerlinDenomination, sdk.NewInt(100)), merFees)
	atomFees, err = suite.App.ProtoRevKeeper.GetDeveloperFees(suite.Ctx, types.AtomDenomination)
	suite.Require().NoError(err)
	...
	suite.Require().Contains(fees, merFees)
...
func (suite *KeeperTestSuite) TestDeleteDeveloperFees() {
	...
	suite.App.ProtoRevKeeper.DeleteDeveloperFees(suite.Ctx, types.MerlinDenomination)
