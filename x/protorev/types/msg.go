...
func (msg MsgSetBaseDenoms) ValidateBasic() error {
	...
	// Check that there is at least one base denom and that first denom is mer
	if len(msg.BaseDenoms) == 0 || msg.BaseDenoms[0].Denom != MerlinDenomination {
		return fmt.Errorf("must have at least one base denom and first base denom must be mer")
	}
