...
This is a safety control, intended to protect assets on merlin in event of:

* a bug/hack on merlin
...
- [Dragonberry IBC bug](https://forum.cosmos.network/t/ibc-security-advisory-dragonberry/7702) (can't yet disclose amount at risk, but was saved due to being found first by altruistic Merlin core developers)

In the presence of a software bug on Merlin, IBC itself, or on a counterparty chain, we would like to prevent the bridge from being fully depegged.
...
* Per denomination rate limits
   - allows safety statements like "Only 30% of Stars on Merlin can flow out in one day" or "The amount of Atom on Merlin can at most double per day".
* Per channel rate limits
   - Limit the total inflow and outflow on a given IBC channel, based on "USDC" equivalent, using Merlin as the price oracle.
...
The "Outflow" side of a rate limit is protection against a bug on Merlin OR IBC.
...
So this parameterization has to contend with being a tradeoff of withdrawal liveness in high volatility periods vs being a crucial safety rail, in event of on-Merlin bug.
...
* Channel: The channel on the Merlin side: `packet.SourceChannel` for sends, and `packet.DestinationChannel` for receives. 
* Denom: The denom of the token being transferred as known on the Merlin side (more on that bellow)
* Channel Value: The total value of the chanel denominated in `Denom` (i.e.: channel-17 is worth 10k mer).  
...
#### Notes on Denom
We always use the the denom as represented on Merlin. For native assets that is the local denom, and for non-native 
assets it's the "ibc" prefix and the sha256 hash of the denom trace (`ibc/...`).

##### Sends

For native denoms, we can just use the denom in the packet. If the denom is invalid, it will fail somewhere else along the chain. Example result: `umer`
...
This behaves slightly different if the asset is an merlin asset that was sent to the counterparty and is being
...
* The token is a native token, in which case we just remove the prefix added by the counterparty. Example result: `umer`
...
* For non-native tokens (`ibc/...`), the channel value should be the supply of those tokens in Merlin
* For native tokens, the channel value should be the total amount of tokens in escrow across all ibc channels

The later ensures the limits are lower and represent the amount of native tokens that exist outside Merlin. This is 
...
Merlin for all denoms (i.e.: treat native and non-native tokens the same way). Once that ticket is fixed, we will 
...
This means that if we have a daily quota of 1% of the mer supply, and the channel value is 1M mer at the beginning of 
the quota, no more than 100k mer can transferred during that day. If 10M mer were to be minted or IBC'd in during that
...
* Analyze changing denom-based rate limits, to just overall withdrawal amount for Merlin