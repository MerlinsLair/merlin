...
use merlin_std_derive::CosmwasmExt;
...
// merlin-std doesn't currently support the SupplyOf query, so I'm defining it localy so it can be used to obtain the channel value
#[derive(
    Clone,
    PartialEq,
    Eq,
...
pub struct QuerySupplyOfResponse {
    ...
    pub amount: ::core::option::Option<merlin_std::types::cosmos::base::v1beta1::Coin>,
}
...
pub mod tests {
    ...
    fn send_native() {
        ...
        assert_eq!(packet.local_denom(&FlowType::Out), "umer");
    }
    ...
    fn receive_native() {
        ...
        let packet = Packet::mock(
            format!("channel-42-counterparty"), // The counterparty's channel is the source here
            format!("channel-17-local"),        // Our channel is the dest channel
            format!("transfer/channel-42-counterparty/umer"),
            0_u128.into(),
        );
        assert_eq!(packet.local_denom(&FlowType::In), "umer");
    }
    ...
    const WRAPPED_OSMO_ON_HUB_TRACE: &str = "transfer/channel-141/umer";
    const WRAPPED_ATOM_ON_MERLIN_TRACE: &str = "transfer/channel-0/uatom";
    const WRAPPED_ATOM_ON_MERLIN_HASH: &str =
    ...
    fn sanity_check() {
        ...
        // uatom sent to merlin
        let packet = Packet::mock(
            format!("channel-141"), // from: hub
            format!("channel-0"),   // to: merlin
            format!("uatom"),
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::In),
            WRAPPED_ATOM_ON_MERLIN_HASH.clone()
        );

        // uatom on merlin sent back to the hub
        let packet = Packet::mock(
            format!("channel-0"),                      // from: merlin
            format!("channel-141"),                    // to: hub
            WRAPPED_ATOM_ON_MERLIN_TRACE.to_string(), // unwrapped before reaching the contract
            0_u128.into(),
        );
        ...
        // mer on the hub sent back to merlin
        // send
        let packet = Packet::mock(
            format!("channel-141"),                // from: hub
            format!("channel-0"),                  // to: merlin
        ...
        // umer on the hub sent back to the merlin
        let packet = Packet::mock(
            format!("channel-141"),                // from: hub
            format!("channel-0"),                  // to: merlin
            WRAPPED_OSMO_ON_HUB_TRACE.to_string(), // unwrapped before reaching the contract
        ...
        // utaom on the merlin sent back to the hub
        // send
        let packet = Packet::mock(
            format!("channel-0"),                      // from: merlin
            format!("channel-141"),                    // to: hub
    ...
    fn sanity_double() {
        ...
        let juno_wrapped_merlin_wrapped_atom_hash =
            "ibc/6CDD4663F2F09CD62285E2D45891FC149A3568E316CE3EBBE201A71A78A69388";

        // Send uatom on stored on merlin to juno
        // send
        let packet = Packet::mock(
            format!("channel-42"),                     // from: merlin
        ...
        let packet = Packet::mock(
            format!("channel-42"), // from: merlin
            format!("channel-0"),  // to: juno
            WRAPPED_ATOM_ON_MERLIN_TRACE.to_string(),
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::In),
            juno_wrapped_merlin_wrapped_atom_hash
        );

        // Send back that multi-wrapped token to merlin
        // send
        let packet = Packet::mock(
            format!("channel-0"),  // from: juno
            format!("channel-42"), // to: merlin
            format!("{}{}", "transfer/channel-0/", WRAPPED_ATOM_ON_MERLIN_TRACE), // unwrapped before reaching the contract
            0_u128.into(),
        );
        assert_eq!(
            packet.local_denom(&FlowType::Out),
    ...
    fn tokenfactory_packet() {
        let json = r#"{"send_packet":{"packet":{"sequence":4,"source_port":"transfer","source_channel":"channel-0","destination_port":"transfer","destination_channel":"channel-1491","data":{"denom":"transfer/channel-0/factory/mer12smx2wdlyttvyzvzg54y2vnqwq2qjateuf7thj/czar","amount":"100000000000000000","sender":"mer1cyyzpxplxdzkeea7kwsydadg87357qnahakaks","receiver":"mer1c584m4lq25h83yp6ag8hh4htjr92d954vklzja"},"timeout_height":{},"timeout_timestamp":1668024476848430980}}}"#;
    ...
    fn packet_with_memo() {
        ...
        let json = r#"{"recv_packet":{"packet":{"sequence":1,"source_port":"transfer","source_channel":"channel-0","destination_port":"transfer","destination_channel":"channel-0","data":{"denom":"stake","amount":"1","sender":"mer177uaalkhra6wth6hc9hu79f72eq903kwcusx4r","receiver":"mer1fj6yt4pwfea4865z763fvhwktlpe020ef93dlq","memo":"some info"},"timeout_height":{"revision_height":100}}}}"#;
