...
func CalculateChannelValue(ctx sdk.Context, denom string, bankKeeper bankkeeper.Keeper) sdk.Int {
	...
	// For non-native (ibc) tokens, return the supply if the token in merlin
...
func (suite *MiddlewareTestSuite) initializeEscrow() (totalEscrow, expectedSed sdk.Int) {
	...
	supply := merlinApp.BankKeeper.GetSupplyWithOffset(suite.chainA.GetContext(), sdk.DefaultBondDenom)
...
func (suite *MiddlewareTestSuite) fullSendTest(native bool) map[string]string {
	...
	channelValue := CalculateChannelValue(suite.chainA.GetContext(), denom, merlinApp.BankKeeper)
...
func (suite *MiddlewareTestSuite) fullRecvTest(native bool) {
	...
	channelValue := CalculateChannelValue(suite.chainA.GetContext(), localDenom, merlinApp.BankKeeper)
...
func (suite *MiddlewareTestSuite) TestFailedSendTransfer() {
	...
	// escrowed := merlinApp.BankKeeper.GetBalance(suite.chainA.GetContext(), escrowAddress, sdk.DefaultBondDenom)
	escrowed := merlinApp.BankKeeper.GetSupplyWithOffset(suite.chainA.GetContext(), sdk.DefaultBondDenom)
