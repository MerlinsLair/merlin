...
collector holds 1000uatom and 2000 umer at Begin Block, and Params'
mintedDenom is set to umer, and AllocationRatio is set to 0.1, 200umer
...
```shell
merlind tx gov submit-proposal update-pool-incentives [gaugeIds] [weights]
```
...
```shell
merlind tx gov submit-proposal update-pool-incentives 2,3 100,200
```
...
```sh
merlind tx poolincentives replace-pool-incentives [gaugeIds] [weights] [flags]
```
...
```bash
merlind tx poolincentives replace-pool-incentives proposal.json --from --chain-id
```
...
```sh
merlind tx poolincentives update-pool-incentives [gaugeIds] [weights] [flags] --from --chain-id
```
...
```bash
merlind tx gov submit-proposal update-pool-incentives proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```sh
merlind query poolincentives distr-info
```
...
```sh
merlind query pool-incentives external-incentivized-gauges
```
...
```sh
merlind query poolincentives gauge-ids [pool-id] [flags]
```
...
```bash
merlind query poolincentives gauge-ids 1
```
...
```sh
merlind query poolincentives incentivized-pools [flags]
```

::: details Example

```bash
merlind query poolincentives incentivized-pools
```
...
```sh
merlind query poolincentives lockable-durations [flags]
```

::: details Example

```bash
merlind query poolincentives lockable-durations
```
...
```sh
merlind query poolincentives params [flags]
```

::: details Example

```bash
merlind query poolincentives params
```
