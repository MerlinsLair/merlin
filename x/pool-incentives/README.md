...
```shell
merlin tx gov submit-proposal update-pool-incentives [gaugeIds] [weights]
```
...
```shell
merlin tx gov submit-proposal update-pool-incentives 2,3 100,200
```
...
```sh
merlin tx poolincentives replace-pool-incentives [gaugeIds] [weights] [flags]
```
...
```bash
merlin tx poolincentives replace-pool-incentives proposal.json --from --chain-id
```
...
```sh
merlin tx poolincentives update-pool-incentives [gaugeIds] [weights] [flags] --from --chain-id
```
...
```bash
merlin tx gov submit-proposal update-pool-incentives proposal.json --from WALLET_NAME --chain-id CHAIN_ID
```
...
```sh
merlin query poolincentives distr-info
```
...
```sh
merlin query poolincentives gauge-ids [pool-id] [flags]
```
...
```bash
merlin query poolincentives gauge-ids 1
```
...
```sh
merlin query poolincentives incentivized-pools [flags]
```

::: details Example

```bash
merlin query poolincentives incentivized-pools
```
...
```sh
merlin query poolincentives lockable-durations [flags]
```

::: details Example

```bash
merlin query poolincentives lockable-durations
```
...
```sh
merlin query poolincentives params [flags]
```

::: details Example

```bash
merlin query poolincentives params
```
