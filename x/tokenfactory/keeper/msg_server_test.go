...
func (suite *KeeperTestSuite) TestMintDenomMsg() {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			amount:    10,
			mintDenom: "factory/mer1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44/evmos",
...
func (suite *KeeperTestSuite) TestBurnDenomMsg() {
	...
	for _, tc := range []struct {
		...
		{
			desc:      "denom does not exist",
			burnDenom: "factory/mer1t7egva48prqmzl59x5ngv4zx0dtrwewc9m7z44/evmos",
			admin:     suite.TestAccs[0].String(),
			valid:     false,
		},
		{
...
func (suite *KeeperTestSuite) TestCreateDenomMsg() {
	defaultDenomCreationFee := types.Params{DenomCreationFee: sdk.NewCoins(sdk.NewCoin("umer", sdk.NewInt(50000000)))}
	for _, tc := range []struct {
		desc                  string
