...
The following information provides a set of guidelines for contributing to the Merlin chain dev repo. Use your best judgment, and, if you see room for improvement, please propose changes to this document.

The contributing guide for Merlin explains the branching structure, how to use the SDK fork, how to make / test updates to SDK branches and how to create release notes.

Contributions come in the form of writing documentation, raising issues / PRs, and any other actions that help develop the Merlin protocol documentation.

## First steps

The first step is to find an issue you want to fix. To identify issues we think are good for first-time contributors, we add the **good first issue** label. [You can see a list of issues to contribute here](https://github.com/merlinslair/merlin/contribute).
...
1. [Fork the repository](https://github.com/merlinslair/merlin).
...
7. Next, rince and repeat the following:
    ...
    3. Create a PR on the Merlin repository. There should be a PR template to help you do so.
...
Note: this section assumes you already have the Go plugin for Vscode installed. Please refer to our [IDE setup docs](https://docs.merlin.zone/developing/merlin-core/ide-guide.html) if you haven't done any IDE setup yet.

Copy the `templates` folder into your `.vscode` folder from our main repo [here](https://github.com/merlinslair/merlin/tree/main/.vscode). This folder has our custom templates for generating tests that fit our testing standards as accurately as possible.
...
where `"[ABSOLUTE PATH TO TEMPLATES FOLDER]"` should look something like: `"User/ExampleUser/merlin/.vscode/templates"`
...
Since the GAMM module is core to the Merlin repo, it might be useful to have a good example of a well-structured GAMM-specific test. This example covers a simple getter function and validates the specific error messages around the function (as opposed to merely the presence of an error):
...
func TestGetPoolAssetsByDenom(t *testing.T) {
    testCases := map[string] struct {
        ...
        "one pool asset": {
            poolAssets: []balancer.PoolAsset {
                {
                    Token:  sdk.NewInt64Coin("umer", 1e12),
            ...
            poolAssets: []balancer.PoolAsset {
                {
                    ...
                    Token:  sdk.NewInt64Coin("umer", 123),
                    Weight: sdk.NewInt(400),
                },
            },
            err: fmt.Errorf(balancer.ErrMsgFormatRepeatingPoolAssetsNotAllowed, "umer"),
        },
    }
...
The e2e package defines an integration testing suite used for full end-to-end testing functionality. This package is decoupled from depending on the Merlin codebase. It initializes the chains for testing via Docker files. 

As a result, the test suite may provide the desired Merlin version to Docker containers during the initialization. This design allows for the opportunity of testing chain upgrades in the future by providing an older Merlin version to the container, performing the chain upgrade, and running the latest test suite. 
...
Additionally, there is an ability to disable certain components of the e2e suite. This can be done by setting the environment variables. See the [E2E test docs](https://github.com/merlinslair/merlin/blob/main/tests/e2e/README.md)  or more details.
...
You can do rm -rf vendor and make build-reproducible to redownload all dependencies - this should pull the latest docker image of Merlin. You should also make sure to do make proto-all to auto-generate your protobuf files. Makes ure you have docker installed.
...
- (Eventually) Make a PR that adds a version handler for the next upgrade
  - [Add v10 upgrade boilerplate #1649](https://github.com/merlinslair/merlin/pull/1649/files)
...
following:
<https://github.com/merlinslair/merlin/blob/8ef2f1845d9c7dd3f422d3f1953e36e5cf112e73/x/mint/types/errors.go#L8-L10>
...
We communicate with various integrators if they'd like release-blocking QA testing for major releases
    * Chainapsis has communicated wanting a series of merlin-frontend functionalities to be checked for correctness on a testnet as a release blocking item
