...
func initAppConfig() (string, interface{}) {
	type MerlinMempoolConfig struct {
	...
	type CustomAppConfig struct {
		...
		MerlinMempoolConfig MerlinMempoolConfig `mapstructure:"merlin-mempool"`
	}
	...
	memCfg := MerlinMempoolConfig{ArbitrageMinGasPrice: "0.01"}

	MerlinAppCfg := CustomAppConfig{Config: *srvCfg, MerlinMempoolConfig: memCfg}

	MerlinAppTemplate := serverconfig.DefaultConfigTemplate + `
###############################################################################
###                      Merlin Mempool Configuration                      ###
###############################################################################
...
func initRootCmd(rootCmd *cobra.Command, encodingConfig params.EncodingConfig) {
	...
	rootCmd.AddCommand(
		...
		forceprune(),
		InitCmd(merlin.ModuleBasics, merlin.DefaultNodeHome),
		...
		AddGenesisWasmMsgCmd(merlin.DefaultNodeHome),
...
// newApp initializes and returns a new Merlin app.
func newApp(logger log.Logger, db dbm.DB, traceStore io.Writer, appOpts servertypes.AppOptions) servertypes.Application {
	...
	return merlin.NewMerlinApp(
		...
		appOpts,
		merlin.GetWasmEnabledProposals(),
		wasmOpts,
...
// createMerlinAppAndExport creates and exports the new Merlin app, returns the state of the new Merlin app for a genesis file.
func createMerlinAppAndExport(
	logger log.Logger, db dbm.DB, traceStore io.Writer, height int64, forZeroHeight bool, jailWhiteList []string,
	appOpts servertypes.AppOptions, modulesToExport []string,
) (servertypes.ExportedApp, error) {
	encCfg := merlin.MakeEncodingConfig() // Ideally, we would reuse the one created by NewRootCmd.
	encCfg.Marshaler = codec.NewProtoCodec(encCfg.InterfaceRegistry)
	loadLatest := height == -1
	homeDir := cast.ToString(appOpts.Get(flags.FlagHome))
	app := merlin.NewMerlinApp(logger, db, traceStore, loadLatest, map[int64]bool{}, homeDir, 0, appOpts, merlin.GetWasmEnabledProposals(), merlin.EmptyWasmOpts)
