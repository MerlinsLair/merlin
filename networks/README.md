...
staking transactions (gentxs) to start the Merlin network.
...
merlin-1 genesis.json is available on ipfs at
...
```{=html}
<!-- The repo contains a copy of the cosmos hub-3 state export, so folks can reproduce the Merlin initial balances in state. -->
```
