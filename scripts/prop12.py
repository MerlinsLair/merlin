...
# This script returns a csv of addresses and the amount of umer to be paid to them 
...
mer_issued = (300000000*1000000*0.45)/365

def dict_mer_rewards_by_gauge_id(data):
  # returns a map of the umer rewards for each gauge id in poolincentives
  ...
  return {gid : (mer_issued*gauge_weights[gid]/total_weight) for gid in gauge_weights}
...
def dict_rewards_by_address(data):
  mer_per_gauge = dict_mer_rewards_by_gauge_id(data)
  ...
  # by multiplying by umer issued to the gauge, and dividing by total locked for the gauge
  mint_by_gauge_addr = {gid : {addr : mer_per_gauge.get(gid, 0)*locked[gid][addr]/gauge_totals[gid] for addr in locked[gid]} for gid in locked}
  ...
  #convert {gauge_id -> {addr -> umer_issued}} into {addr -> umer_issued} by merging the dictionaries for each gauge id, adding the mer issued for each address
...
def compute_sanity_check_and_export(data):
  ...
  community_paid = dict_mer_rewards_by_gauge_id(data)["0"]

  print("expected total: ", int(mer_issued), "umer")
  print("paid to LPers: ", int(total_paid), "umer")
  print("paid to community pool: ", int(community_paid), "umer")
  imbalance = int(mer_issued - (total_paid + community_paid))
