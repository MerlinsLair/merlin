...
# 3) update MERLIN_E2E_UPGRADE_VERSION variable in .vscode/launch.json
...
# Github workflow which calls this script can be found here: merlin/.github/workflows/auto-update-upgrade.yml
...
# constants.go logic
echo "// UpgradeName defines the on-chain upgrade name for the Merlin $version_create upgrade." >> $CONSTANTS_FILE
...
PREV_MERLIN_DEV_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/merlabs/merlin-dev/tags?page=1&page_size=100'            | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")
PREV_MERLIN_E2E_TAG=$(curl -L -s 'https://registry.hub.docker.com/v2/repositories/merlabs/merlin-e2e-init-chain/tags?page=1&page_size=100' | jq -r '.results[] | .name | select(.|test("^(?:v|)[0-9]+\\.[0-9]+(?:$|\\.[0-9]+$)"))' | grep --max-count=1 "")

# previousVersionMerTag  = PREV_MERLIN_DEV_TAG
if [[ $version_create == v$(($(echo $PREV_MERLIN_DEV_TAG | awk -F . '{print $1}')+1)) ]]; then	
    echo "Found previous merlin-dev tag $PREV_MERLIN_DEV_TAG"
	sed -i '/previousVersionMerTag/s/".*"/'"\"$PREV_MERLIN_DEV_TAG\""'/' $e2e_file
else
    PREV_MERLIN_DEV_TAG=v$((${version_create:1}-1)).0.0
    echo "Using pre-defined merlin-dev tag: $PREV_MERLIN_DEV_TAG"
    sed -i '/previousVersionMerTag/s/".*"/'"\"$PREV_MERLIN_DEV_TAG\""'/' $e2e_file
fi

# previousVersionInitTag  = PREV_MERLIN_E2E_TAG
if [[ $version_create == v$(($(echo $PREV_MERLIN_E2E_TAG | awk -F . '{print $1}' | grep -Eo '[0-9]*')+1)) ]]; then	
    echo "Found previous merlin-e2e-init-chain tag $PREV_MERLIN_E2E_TAG"
	sed -i '/previousVersionInitTag/s/".*"/'"\"$PREV_MERLIN_E2E_TAG\""'/' $e2e_file
else
    PREV_MERLIN_E2E_TAG=v$((${version_create:1}-1)).0.0
    echo "Using pre-defined merlin-e2e-init-chain tag: $PREV_MERLIN_E2E_TAG"
    sed -i '/previousVersionInitTag/s/".*"/'"\"$PREV_MERLIN_E2E_TAG\""'/' $e2e_file
fi

# update MERLIN_E2E_UPGRADE_VERSION in launch.json
sed -i "s/${bracks}MERLIN_E2E_UPGRADE_VERSION${bracks}: ${bracks}v$latest_version${bracks}/${bracks}MERLIN_E2E_UPGRADE_VERSION${bracks}: ${bracks}$version_create${bracks}/" ./.vscode/launch.json
