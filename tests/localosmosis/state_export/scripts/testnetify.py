...
# Contants
BONDED_TOKENS_POOL_MODULE_ADDRESS = "mer1fl48vsnmsdzcv85q5d2q4z5ajdha8yu3aq6l09"
DISTRIBUTION_MODULE_ADDRESS = "mer1jv65s3grqf6v6jl3dp4t6c9t9rk99cd80yhvld"
...
def main():
    ...
    old_validator = Validator(
        ...
        operator_address = "mervaloper1cyw4vw20el8e7ez8080md0r8psg25n0cq98a9n",
        consensus_address = "mervalcons1z6skn9g6s7py0klztr7acutr3anqd52k9x5p70"
    )
    ...
    old_account = Account(
        ...
        address = "mer1cyw4vw20el8e7ez8080md0r8psg25n0c6j07j5"
    )
        ...
        if balance['address'] == new_account.address:
            for coin in balance['coins']:
                if coin['denom'] == "umer":
                    ...
                    if not args.quiet:
                        print("\tUpdate {} umer balance to {}".format(new_account.address, coin["amount"]))
                    break
            break
    
    # Add 1 BN umer to bonded_tokens_pool module address
    for balance in genesis['app_state']['bank']['balances']:
        if balance['address'] == BONDED_TOKENS_POOL_MODULE_ADDRESS:
            ...
            for coin in balance['coins']:
                if coin['denom'] == "umer":
                    coin["amount"] = str(int(coin["amount"]) + 1000000000000000)
                    if not args.quiet:
                        print("\tUpdate {} (bonded_tokens_pool_module) umer balance to {}".format(BONDED_TOKENS_POOL_MODULE_ADDRESS, coin["amount"]))
                    break
            break
        ...
        if balance['address'] == DISTRIBUTION_MODULE_ADDRESS:
            ...
            for coin in balance['coins']:
                if coin['denom'] == "umer":
                    ...
                    if not args.quiet:
                        print("\tUpdate {} (distribution_module) umer balance to {}".format(DISTRIBUTION_MODULE_ADDRESS, coin["amount"]))
                    break
            break

    # Update bank balance 
    for supply in genesis['app_state']['bank']['supply']:
        if supply["denom"] == "umer":
            if not args.quiet:
                print("\tUpdate total umer supply from {} to {}".format(supply["amount"], str(int(supply["amount"]) + 2000000000000000 - DISTRIBUTION_MODULE_OFFSET)))
