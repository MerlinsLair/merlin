...
func SetupTestingApp() (ibctesting.TestingApp, map[string]json.RawMessage) {
	merlinApp := app.Setup(false)
	return merlinApp, app.NewDefaultGenesisState()
}
...
func (chain *TestChain) MoveEpochsToTheFuture() error {
	epochsKeeper := chain.GetMerlinApp().EpochsKeeper
...
// GetMerlinApp returns the current chain's app as an MerlinApp
func (chain *TestChain) GetMerlinApp() *app.MerlinApp {
	v, _ := chain.App.(*app.MerlinApp)
	return v
}
