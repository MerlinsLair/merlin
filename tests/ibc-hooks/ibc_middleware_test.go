...
func (suite *HooksTestSuite) TestOnRecvPacketHooks() {
	...
	testCases := []struct {
		...
		{"override", func(status *testutils.Status) {
			suite.chainB.GetMerlinApp().TransferStack.
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.msg, func() {
			...
			ack := suite.chainB.GetMerlinApp().TransferStack.
			...
			if _, ok := suite.chainB.GetMerlinApp().TransferStack.
...
func (suite *HooksTestSuite) receivePacketWithSequence(receiver, memo string, prevSequence uint64) []byte {
	...
	err := suite.chainB.GetMerlinApp().HooksICS4Wrapper.SendPacket(
...
func (suite *HooksTestSuite) TestPacketsThatShouldBeSkipped() {
	...
	testCases := []struct {
		...
		{`{"wasm": {"contract": "mer1clpqr4nrk4khgkxj78fcwwh6dl3uw4epasmvnj"}}`, false},
		{`{"wasm": {"msg": "something"}}`, false},
		// invalid receiver
		{`{"wasm": {"contract": "mer1clpqr4nrk4khgkxj78fcwwh6dl3uw4epasmvnj", "msg": {}}}`, false},
		// msg not an object
...
func (suite *HooksTestSuite) SetupPools(chainName Chain, multipliers []sdk.Dec) []gammtypes.CFMMPoolI {
	...
	bondDenom := chain.GetMerlinApp().StakingKeeper.BondDenom(chain.GetContext())
	...
	for index, multiplier := range multipliers {
		...
		poolId, err := chain.GetMerlinApp().PoolManagerKeeper.CreatePool(chain.GetContext(), msg)
		suite.Require().NoError(err)

		pool, err := chain.GetMerlinApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
		suite.Require().NoError(err)
...
func (suite *HooksTestSuite) SetupCrosschainSwaps(chainName Chain) (sdk.AccAddress, sdk.AccAddress) {
	...
	amounts := sdk.NewCoins(sdk.NewCoin("umer", i), sdk.NewCoin("stake", i), sdk.NewCoin("token0", i), sdk.NewCoin("token1", i))
	...
	channels := `[["mer", "channel-0"],["juno", "channel-0"]]`
...
func (suite *HooksTestSuite) TestCrosschainSwaps() {
	...
	merlinApp := suite.chainA.GetMerlinApp()
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(merlinApp.WasmKeeper)

	balanceSender := merlinApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")

	ctx := suite.chainA.GetContext()

	msg := fmt.Sprintf(`{"merlin_swap":{"swap_amount": "1000","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
	...
	balanceSender2 := merlinApp.BankKeeper.GetBalance(suite.chainA.GetContext(), owner, "token0")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCTest() {
	...
	merlinAppB := suite.chainB.GetMerlinApp()
	balanceToken0 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	receiver := initializer
	balanceToken1 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount":"1000","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing", "next_memo":{}}}`,
		receiver,
	)
	// Generate full memo
	...
	balanceToken0After := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	suite.Require().Equal(int64(1000), balanceToken0.Amount.Sub(balanceToken0After.Amount).Int64())

	balanceToken1After := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiver, token1IBC)
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadAck() {
	...
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount":"1000","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s","on_failed_delivery": {"local_recovery_addr": "%s"}}}`,
	...
	merlinAppA := suite.chainA.GetMerlinApp()
	balanceContract := merlinAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), crosschainAddr, "token1")
	...
	recoverMsg := `{"recover": {}}`
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(merlinAppA.WasmKeeper)
	_, err = contractKeeper.Execute(suite.chainA.GetContext(), crosschainAddr, recoverAddr, []byte(recoverMsg), sdk.NewCoins())
	suite.Require().NoError(err)

	balanceRecovery := merlinAppA.BankKeeper.GetBalance(suite.chainA.GetContext(), recoverAddr, "token1")
...
func (suite *HooksTestSuite) TestCrosschainSwapsViaIBCBadSwap() {
	...
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount":"1000","output_denom":"token1","slippage":{"min_output_amount":"50000"},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)
	// Generate full memo
	msg := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": %s } }`, crosschainAddr, swapMsg)
...
func (suite *HooksTestSuite) TestBadCrosschainSwapsNextMemoMessages() {
	...
	innerMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount":"10","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s","on_failed_delivery": {"local_recovery_addr": "%s"},"next_memo":%%s}}`,
		receiver, // Note that this is the chain A account, which does not exist on chain B
		recoverAddr)
...
func (suite *HooksTestSuite) CreateIBCPoolOnChainB() {
	...
	_, err = chain.GetMerlinApp().GAMMKeeper.GetPoolAndPoke(chain.GetContext(), poolId)
	suite.Require().NoError(err)
...
func (suite *HooksTestSuite) TestCrosschainForwardWithMemo() {
	...
	balanceToken0IBCBefore := suite.chainA.GetMerlinApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
	...
	// Generate swap instructions for the contract
	nextMemo := fmt.Sprintf(`{"wasm": {"contract": "%s", "msg": {"merlin_swap":{"swap_amount":"800","output_denom":"token0","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}}}`,
		crosschainAddrB,
		receiver,
	)
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount":"1000","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing", "next_memo": %s}}`,
		crosschainAddrB,
		nextMemo,
	)
	fmt.Println(swapMsg)
	...
	balanceToken0IBCAfter := suite.chainA.GetMerlinApp().BankKeeper.GetBalance(suite.chainA.GetContext(), receiver, token0IBC)
...
func (suite *HooksTestSuite) ExecuteOutpostSwap(initializer, receiverAddr sdk.AccAddress, receiver string) {
	...
	outpostAddr := suite.chainB.InstantiateContract(&suite.Suite,
		fmt.Sprintf(`{"crosschain_swaps_contract": "%s", "merlin_channel": "channel-0"}`, crosschainAddr), 1)
	...
	merlinAppB := suite.chainB.GetMerlinApp()
	balanceToken0 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), initializer, token0IBC)
	balanceToken1 := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)

	suite.Require().Equal(int64(0), balanceToken1.Amount.Int64())

	// Generate swap instructions for the contract
	swapMsg := fmt.Sprintf(`{"merlin_swap":{"swap_amount": "1000","output_denom":"token1","slippage":{"twap": {"window_seconds": 1, "slippage_percentage":"20"}},"receiver":"%s", "on_failed_delivery": "do_nothing"}}`,
		receiver,
	)

	// Call the outpost
	contractKeeper := wasmkeeper.NewDefaultPermissionKeeper(merlinAppB.WasmKeeper)
	...
	balanceToken1After := merlinAppB.BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, token1IBC)
