...
import (
	...
	"github.com/merlinslair/merlin/tests/e2e/configurer/chain"
	"io"
	"os"
	"path/filepath"
	"strconv"
...
func (s *IntegrationTestSuite) TestConcentratedLiquidity() {
	...
	fundTokens := []string{"1000000umer", "1000000uion", "1000000stake"}
...
func (s *IntegrationTestSuite) TestGeometricTwapMigration() {
	...
	umerIn := fmt.Sprintf("1000000%s", "umer")

	swapWalletAddr := node.CreateWallet(migrationWallet)

	node.BankSend(umerIn, chainA.NodeConfigs[0].PublicAddress, swapWalletAddr)

	// Swap to create new twap records on the pool that was created pre-upgrade.
	node.SwapExactAmountIn(umerIn, minAmountOut, fmt.Sprintf("%d", oldPoolId), otherDenom, swapWalletAddr)
}
...
func (s *IntegrationTestSuite) TestIBCTokenTransferRateLimiting() {
	...
	merSupply, err := node.QuerySupplyOf("umer")
	s.NoError(err)

	f, err := merSupply.ToDec().Float64()
	s.NoError(err)

	over := f * 0.02

	// Sending >1%
	chainA.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.MerDenom, int64(over)))
	...
	node.InstantiateWasmContract(
		...
		fmt.Sprintf(`{"gov_module": "%s", "ibc_module": "%s", "paths": [{"channel_id": "channel-0", "denom": "%s", "quotas": [{"name":"testQuota", "duration": 86400, "send_recv": [1, 1]}] } ] }`, node.PublicAddress, node.PublicAddress, initialization.MerToken.Denom),
	...
	s.Eventually(
		...
		1*time.Minute,
		10*time.Millisecond,
		"Merlin node failed to retrieve params",
	)

	// Sending <1%. Should work
	chainA.SendIBC(chainB, chainB.NodeConfigs[0].PublicAddress, sdk.NewInt64Coin(initialization.MerDenom, 1))
	// Sending >1%. Should fail
	node.FailIBCTransfer(initialization.ValidatorWalletName, chainB.NodeConfigs[0].PublicAddress, fmt.Sprintf("%dumer", int(over)))

	// Removing the rate limit so it doesn't affect other tests
	node.WasmExecute(contracts[0], `{"remove_path": {"channel_id": "channel-0", "denom": "umer"}}`, initialization.ValidatorWalletName)
}
...
func (s *IntegrationTestSuite) TestIBCWasmHooks() {
	...
	nodeB.SendIBCTransfer(validatorAddr, contractAddr, fmt.Sprintf("%dumer", transferAmount),
		fmt.Sprintf(`{"wasm":{"contract":"%s","msg": {"increment": {}} }}`, contractAddr))

	// check the balance of the contract
	denomTrace := transfertypes.ParseDenomTrace(transfertypes.GetPrefixedDenom("transfer", "channel-0", "umer"))
	ibcDenom := denomTrace.IBCDenom()
	s.CheckBalance(nodeA, contractAddr, ibcDenom, transferAmount)

	// sender wasm addr
	senderBech32, err := ibchookskeeper.DeriveIntermediateSender("channel-0", validatorAddr, "mer")
...
func (s *IntegrationTestSuite) TestPacketForwarding() {
	...
	nodeA.SendIBCTransfer(validatorAddr, validatorAddr, fmt.Sprintf("%dumer", transferAmount), string(forwardMemo))

	// check the balance of the contract
	s.CheckBalance(nodeA, contractAddr, "umer", transferAmount)
...
func (s *IntegrationTestSuite) TestArithmeticTWAP() {
	...
	// 2000000uion for some amount of umer, and
	// 2000000umer for some amount of stake
	...
	// to the benefit (positive price impact) of the other two assets (A&C, or stake and umer)
...
func (s *IntegrationTestSuite) TestStateSync() {
	...
	tempDir, err := os.MkdirTemp("", "merlin-e2e-statesync-")
	s.Require().NoError(err)
...
// The pool is created with 1_000_000 umer and 2_000_000 stake and equal weights.
// Assuming base asset is umer, the initial twap is 2
// Upon swapping 1_000_000 umer for stake, supply changes, making umer less expensive.
// As a result of the swap, twap changes to 0.5.
func (s *IntegrationTestSuite) TestGeometricTWAP() {
	const (
		// This pool contains 1_000_000 umer and 2_000_000 stake.
		// Equals weights.
		poolFile   = "geometricPool.json"
		walletName = "geometric-twap-wallet"

		denomA = "umer" // 1_000_000 umer
	...
	// stake out = stake supply * (1 - (umer supply before / umer supply after)^(umer weight / stake weight))
	...
	// umer = 1_000_000 + 1_000_000 = 2_000_000
	...
	// We swap umer so umer's supply will increase and stake will decrease.
	...
	// quote assset supply / base asset supply = 2_000_000 / 1_000_000 = 2
	// Next, we swapped 1_000_000 umer for stake.
	// Now, we roughly have
	// uatom = 1_000_000
	// umer = 2_000_000
	// quote assset supply / base asset supply = 1_000_000 / 2_000_000 = 0.5
