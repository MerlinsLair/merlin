...
func (m *Manager) RunHermesResource(chainAID, merARelayerNodeName, merAValMnemonic, chainBID, merBRelayerNodeName, merBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				...
				fmt.Sprintf("OSMO_A_E2E_VAL_MNEMONIC=%s", merAValMnemonic),
				fmt.Sprintf("OSMO_B_E2E_VAL_MNEMONIC=%s", merBValMnemonic),
				fmt.Sprintf("OSMO_A_E2E_VAL_HOST=%s", merARelayerNodeName),
				fmt.Sprintf("OSMO_B_E2E_VAL_HOST=%s", merBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
